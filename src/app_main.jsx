...
function App() {
  ...
  // Slots
  const [slots, setSlots] = useState(EMPTY_SLOTS(tweaks.slotCount));
  const [scanning, setScanning] = useState(null); // index or null
  const [copied, setCopied] = useState(null); // {type:'slot'|'all', key}
  ...
  // slot count change resets slots (keeps any filled in first N)
  useEffect(() => {
    setSlots((prev) => {
      const next = EMPTY_SLOTS(tweaks.slotCount);
      for (let i = 0; i < Math.min(prev.length, tweaks.slotCount); i++) {
        next[i] = prev[i];
      }
      return next;
    });
  ...
  const copySlot = (i) => {
    const s = slots[i];
    if (!s.code) return;
    const val = trimRight(s.code, tweaks.trimDigits);
  ...
  const saveFile = () => {
    ...
    const filename = `${company.replace(/\s+/g,'_')}_${refId.replace(/\s+/g,'_')}_${stamp.replace(/[: ]/g,'-')}.txt`;
    const body = [
      `# Trim Code export`,
      `COMPANY : ${company}`,
      `REF-ID  : ${refId}`,
      `SAVED   : ${stamp}`,
      `TRIM    : last ${tweaks.trimDigits} digits`,
      `-----`,
      ...slots.map((s, i) => {
        const tag = String(i+1).padStart(2,'0');
        if (!s.code) return `${tag}  --`;
        return `${tag}  ${trimRight(s.code, tweaks.trimDigits)}    (full: ${s.code})`;
      }),
      `-----`,
      `END`
    ].join('\n');
  ...
  return (
    <div className={`app layout-${tweaks.layout}`}>
      ...
      <main className="main">
        ...
        <Sidebar savedFiles={savedFiles} onRedownload={(rec) => {
          const blob = new Blob([rec.body], { type: 'text/plain' });
          const url = URL.createObjectURL(blob);
          const a = document.createElement('a');
...
function Slot({ index, slot, showPreview, trimDigits, onTap, onCopy, onClear, copied }) {
  ...
  return (
    <div className={`slot ${filled?'is-filled':'is-empty'}`}>
      ...
      <button className="slot-body" onClick={filled ? onTap : onTap}>
        {filled ? (
          <>
  ...
  return (
    <div className="actionbar">
      ...
      <div className="ab-spacer">
        ...
        {readyToSave && <span className="hint ok">ready — saves as .txt in your downloads</span>}
      </div>
    ...
    <aside className="sidebar">
      ...
      {savedFiles.length === 0 && (
        <div className="side-empty">
          ...
          <div className="se-hint">Saved .txt files will appear here<br/>and in your downloads folder.</div>
        </div>
      )}
      ...
      <div className="side-foot">
        <div className="sf-row"><span className="k">DIR</span><span className="v">~/Downloads/trim-code/</span></div>
