/* eslint-disable */
// Scan modal with animated scanning scene

const { useState: useStateM, useEffect: useEffectM, useRef: useRefM } = React;

function ScanModal({ slotIndex, onComplete, onCancel, showPreview, trimDigits, beepOnScan }) {
  const [phase, setPhase] = useStateM('aiming'); // aiming | found | confirm
  const [code, setCode] = useStateM('');
  const [progress, setProgress] = useStateM(0);
  const [flickerChars, setFlickerChars] = useStateM('');

  // fake detection after ~2s
  useEffectM(() => {
